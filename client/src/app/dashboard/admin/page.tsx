        ...
        <div className="space-y-8">
            ...
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {metrics.map(({ label, value, change, icon: Icon }) => (
                    <div key={label} className="rounded-2xl border border-white/[0.08] bg-[#0e0f12] p-5">
                        <div className="flex items-center justify-between"><span className="text-sm text-white/50">{label}</span><Icon className="h-5 w-5 text-red-500" /></div>
            ...
            <section className="grid gap-6 lg:grid-cols-[1.35fr_1fr]">
                <div className="rounded-2xl border border-white/[0.08] bg-[#0e0f12] p-5 md:p-6">
                    <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-white">District Activity</h2><p className="mt-1 text-xs text-white/40">Member activity across the branch network</p></div><Activity className="h-5 w-5 text-red-500" /></div>
                    <div className="mt-8 flex h-44 items-end gap-2 border-b border-white/[0.08] px-2">{[42, 58, 48, 72, 64, 88, 76, 94, 68, 82, 56, 74].map((height, index) => <div key={index} className="flex-1 rounded-t-md bg-red-500/80 transition hover:bg-red-400" style={{ height: `${height}%` }} />)}</div>
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-[#0e0f12] p-5 md:p-6"><h2 className="text-lg font-bold text-white">Platform Health</h2><p className="mt-1 text-xs text-white/40">Real-time service status</p><div className="mt-7 space-y-5">{[["API Services", "Operational", "99.98%"], ["Socket.IO", "Operational", "42ms latency"], ["AI Coach", "Operational", "1.2s response"]].map(([name, status, value]) => <div key={name} className="flex items-center justify-between border-b border-white/[0.06] pb-4"><div><p className="text-sm font-semibold text-white">{name}</p><p className="mt-1 flex items-center gap-2 text-xs text-emerald-400"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />{status}</p></div><span className="text-xs text-white/40">{value}</span></div>)}</div></div>
            </section>

            <section className="grid gap-6 md:grid-cols-3">
                {quickActions.map(([title, description, action, Icon]) => <div key={title as string} className="rounded-2xl border border-white/[0.08] bg-[#0e0f12] p-5"><Icon className="h-5 w-5 text-red-500" /><h2 className="mt-4 text-base font-bold text-white">{title as string}</h2><p className="mt-2 min-h-10 text-sm leading-5 text-white/45">{description as string}</p><button className="mt-5 text-xs font-bold uppercase tracking-widest text-red-400 hover:text-red-300">{action as string} <ArrowUpRight className="ml-1 inline h-3.5 w-3.5" /></button></div>)}
            </section>
        </div>
    );
}