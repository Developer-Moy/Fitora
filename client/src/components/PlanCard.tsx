    ...
    >
      ...
      <div>
        <div className="h-7 mb-4 flex items-center justify-between">
          {badge ? (
            <span
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-wider uppercase ${isRedAccent
          ...
          <ul className="space-y-3">
            {features.map((feature, idx) => (
              <li key={idx} className="flex items-start gap-3">
                <div
                  className={`mt-0.5 p-1 rounded-full shrink-0 ${isRedAccent
      ...
      <button
        onClick={() => onSelect(name, finalPrice)}
        disabled={isCurrentPlan}
        className={`relative z-10 w-full h-12 rounded-xl font-black text-xs tracking-widest uppercase transition-all duration-300 active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2 ${isCurrentPlan
            ? "bg-[#1E293B]/50 text-[#64748B] border border-[#1E293B] cursor-default"
            : buttonGradient
          }`}
      >
        <span>
        ...
        {!isCurrentPlan && <Zap className="w-3.5 h-3.5 fill-current" />}
      </button>
    </motion.div>
  );
}