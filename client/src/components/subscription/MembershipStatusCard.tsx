...
const PROGRESS_COLOR_CLASS: Record<MembershipStatus, string> = {
  active: "bg-gradient-to-r from-emerald-500 to-emerald-300",
...
const CARD_BORDER_CLASS: Record<MembershipStatus, string> = {
  ...
  expired: "border-rose-500/40 shadow-[0_0_30px_rgba(244,63,94,0.15)]",
};
...
export default function MembershipStatusCard({
  ...
  const progress = isFree
    ? 0
    : calculateProgressPercentage(start, expiry, now);
  ...
  return (
    <div
      className={`w-full bg-black border rounded-3xl p-6 sm:p-8 shadow-[0_0_30px_rgba(0,0,0,0.3)] relative overflow-hidden ${
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2.5">
            <CalendarClock className="w-4 h-4 sm:w-5 sm:h-5 text-white/60" />
            <h2 className="text-xl sm:text-2xl font-black uppercase tracking-tight text-white font-sans">
              Membership Status
            </h2>
          </div>
          <span
            className={`inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full text-[11px] sm:text-[12px] font-black uppercase tracking-wider border ${
              isFree
          ...
          <div className="space-y-4 pt-2">
            ...
            {onRenew && (
              <button
                type="button"
                onClick={onRenew}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-white text-black border border-white font-bold text-xs sm:text-sm px-5 py-2.5 rounded-full hover:bg-neutral-100 hover:shadow-[0_0_25px_rgba(255,255,255,0.4)] transition-all shadow-xl cursor-pointer"
              >
        ...
        {!isFree && (
          <div className="space-y-5 pt-2">
            <div>
              ...
              <p
                className={`mt-1 text-2xl xs:text-3xl sm:text-4xl font-black font-mono text-center sm:text-left tracking-tight ${COUNTDOWN_TEXT_CLASS[status]}`}
              >
                {remaining.isExpired
                  ? "Membership Expired"
            ...
            <div className="flex items-center justify-between flex-wrap gap-3 pt-1">
              <div className="flex items-center gap-2 text-xs text-white/60">
                ...
                <span>Expires:</span>
                <strong className="text-white font-medium">{expiryLabel}</strong>
              </div>
              ...
              {remaining.isExpired ? (
                <button
                  type="button"
                  onClick={onRenew}
                  className="inline-flex items-center justify-center gap-2 bg-rose-500 text-white border border-rose-400 font-black text-xs sm:text-sm px-5 py-2.5 rounded-full hover:bg-rose-600 hover:shadow-[0_0_25px_rgba(244,63,94,0.4)] transition-all shadow-lg cursor-pointer"
                >
                ...
                <button
                  type="button"
                  onClick={onRenew}
                  className="inline-flex items-center justify-center gap-2 bg-amber-400 text-black border border-amber-500 font-black text-xs sm:text-sm px-5 py-2.5 rounded-full hover:bg-amber-300 hover:shadow-[0_0_25px_rgba(245,158,11,0.35)] transition-all shadow-lg cursor-pointer"
                >
