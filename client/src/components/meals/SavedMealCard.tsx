...
const SavedMealCard = ({
  ...
  const handleCopyRecipe = async () => {
    const recipeText = `FITORA SAVED MEAL: ${name}
...
Key Ingredients: ${ingredients.join(", ")}`;
    ...
    try {
      ...
      toast.success(`${name} recipe & macros copied!`);
    } catch {
    ...
    <>
      ...
      <div className="group relative bg-neutral-950 border border-white/10 rounded-2xl overflow-hidden shadow-2xl hover:border-white/30 transition-all duration-500 flex flex-col h-full select-none">
        ...
        <div className="p-5 flex flex-col grow space-y-3 -mt-2 relative z-10 bg-neutral-950">
          ...
          {/* Action Button Footer (Only View Details) */}
          <div className="pt-2.5 border-t border-white/10 flex items-center justify-end">
            <button
              type="button"
              onClick={() => setIsModalOpen(true)}
              className="w-full group/btn inline-flex items-center justify-center gap-1.5 sm:gap-2 bg-white text-black border border-white font-extrabold text-[11px] sm:text-xs px-3 sm:px-4 py-2.5 rounded-full hover:bg-neutral-100 hover:shadow-[0_0_20px_rgba(255,255,255,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 shadow-xl cursor-pointer"
            >
        ...
        >
          ...
          >
            ...
            <div className="p-5 sm:p-6 space-y-3.5 flex-1 overflow-y-auto">
              ...
              {/* Mobile Action Button (Only Copy Recipe) */}
              <div className="pt-2 flex flex-col gap-2">
                <button
                  type="button"
                  onClick={handleCopyRecipe}
                  className="w-full inline-flex items-center justify-center gap-2 bg-white text-black font-extrabold hover:bg-neutral-100 transition-all py-2.5 rounded-full uppercase text-xs tracking-wider cursor-pointer shadow-lg active:scale-95"
                >
          ...
          >
            ...
            <div className="grid grid-cols-12 items-stretch min-h-95 lg:min-h-105">
              ...
              <div className="col-span-7 p-8 flex flex-col justify-between space-y-4">
                ...
                {/* Action Buttons (Only Copy Recipe) */}
                <div className="pt-2 flex">
                  <button
                    type="button"
                    onClick={handleCopyRecipe}
                    className="w-full inline-flex items-center justify-center gap-2 bg-white text-black font-extrabold hover:bg-neutral-100 transition-all px-6 py-3 rounded-full uppercase text-xs tracking-wider cursor-pointer shadow-xl active:scale-95"
                  >
                    <Copy className="w-3.5 h-3.5" />
                    <span>Copy Recipe</span>
                  </button>
