...
export default function MembershipExpiryBanner({
  ...
  const statusConfigs: Record<MembershipBannerStatus, StatusConfig> = {
    expiring_soon: {
      ...
      defaultMessage:
        daysRemaining !== undefined
          ? `Your membership plan will expire in ${daysRemaining} day${
              daysRemaining === 1 ? "" : "s"
            }${
          ...
          : `Your membership is approaching its renewal date${
              expiryDate ? ` on ${expiryDate}` : ""
    ...
    expired: {
      defaultTitle: planName ? `${planName} Expired` : "Membership Expired",
      defaultBadge: "Expired",
      defaultMessage: `Your membership access has lapsed${
        expiryDate ? ` as of ${expiryDate}` : ""
  ...
  const renderedActionLabel = actionLabel ?? currentConfig.defaultActionLabel;
  ...
  return (
    <AnimatePresence>
      <motion.aside
        ...
        className={`w-full rounded-2xl transition-all select-none relative overflow-hidden ${
          isLight
          ...
          <div className="flex items-start gap-4 flex-1 min-w-0">
            ...
            <div className="space-y-1.5 min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2.5 flex-wrap">
                <h3
                  className={`text-base sm:text-lg font-black uppercase tracking-tight font-sans ${
              ...
              {/* Message Description */}
              <p
                className={`text-xs sm:text-sm font-medium leading-relaxed max-w-3xl ${
              ...
              {expiryDate && status !== "no_membership" && (
                <div
                  className={`inline-flex items-center gap-1.5 text-[11px] font-semibold pt-1 ${
          ...
          <div className="flex items-center gap-3 shrink-0 flex-wrap sm:flex-nowrap pt-2 md:pt-0 self-start md:self-center">
              ...
              <>
                {secondaryActionHref ? (
                  <Link
                    ...
                    className={`inline-flex items-center justify-center font-bold text-xs sm:text-sm px-4 py-2.5 rounded-full border transition-all cursor-pointer ${
                      isLight
            ...
            {actionHref ? (
              <Link
                href={actionHref}
                onClick={onAction}
                className={`inline-flex items-center justify-center gap-1.5 font-bold text-xs sm:text-sm px-5 py-2.5 rounded-full transition-all cursor-pointer shadow-md group ${
                  isLight
